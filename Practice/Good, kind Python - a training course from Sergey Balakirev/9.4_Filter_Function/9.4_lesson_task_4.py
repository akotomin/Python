...
fist_lst = input().split()
second_lst = input().split()


filtred_first = set(filter(lambda x: int(x) % 2 == 0, fist_lst))
filtred_second = set(filter(lambda x: int(x) % 2 == 0, second_lst))
lst = sorted(list(filtred_first & filtred_second), reverse=True)
