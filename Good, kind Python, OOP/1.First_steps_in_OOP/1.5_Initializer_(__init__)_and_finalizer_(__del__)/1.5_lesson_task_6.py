...
# Если локальное свойство is_show равно False, то методы show_table(), show_graph() и show_bar() должны
# выводить сообщение: "Отображение данных закрыто"
...
class Graph:
    ...
    def show_graph(self):
        if not self.is_show:
            return print("Отображение данных закрыто")
        return print(f"Графическое отображение данных: {' '.join(map(str, self.data))}")

    def show_bar(self):
        if not self.is_show:
            return print("Отображение данных закрыто")
        return print("Столбчатая диаграмма:", *self.data)
